...
import { buildSshTerminalCommand } from '../services/shellPath';
...
export function registerCommands(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.refresh', () => {
    ...
    // Toggle sort order
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.toggleSortOrder', () => {
    ...
    // Create session
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.createSession', async (item?: TmuxTreeItem) => {
    ...
    // Delete sessions (supports multi-select batch deletion)
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.deleteSession', async (item: TmuxTreeItem, selectedItems?: TmuxTreeItem[]) => {
            // Get the list of sessions to delete
            const sessionsToDelete: TmuxTreeItem[] = [];
            
            if (selectedItems && selectedItems.length > 1) {
                // Multi-select mode: filter all session items
    ...
    // Rename session
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.renameSession', async (item: TmuxTreeItem) => {
    ...
    // Attach to session
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.attachSession', async (item: TmuxTreeItem) => {
            if (!item.data.session) {
                return;
            }
            ...
            if (connection?.type === 'ssh' && connection.config) {
                const sshCmd = buildSshTerminalCommand(connection.config, attachCmd);
    ...
    // Quick attach session (triggered from status bar)
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.quickAttach', async () => {
            // Get all connections
            ...
            // Iterate all connections to get sessions
            ...
            if (selected.isSSH && selected.sshConfig) {
                const sshCmd = buildSshTerminalCommand(selected.sshConfig, attachCmd);
                terminal = vscode.window.createTerminal({
    ...
    // Create window
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.createWindow', async (item: TmuxTreeItem) => {
            if (!item.data.session) {
                return;
            }
    ...
    // Delete windows (supports multi-select batch deletion)
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.deleteWindow', async (item: TmuxTreeItem, selectedItems?: TmuxTreeItem[]) => {
            // Get the list of windows to delete
            const windowsToDelete: TmuxTreeItem[] = [];
            
            if (selectedItems && selectedItems.length > 1) {
                // Multi-select mode: filter all window items
                for (const selected of selectedItems) {
            ...
            // Sort by window index descending and delete from back to front to avoid index shifts
    ...
    // Rename window
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.renameWindow', async (item: TmuxTreeItem) => {
    ...
    // Switch to window
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.selectWindow', async (item: TmuxTreeItem) => {
            if (!item.data.window || !item.data.session) {
                return;
            }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.splitPaneHorizontal', async (item: TmuxTreeItem) => {
            let target: string;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.splitPaneVertical', async (item: TmuxTreeItem) => {
            let target: string;
    ...
    // Switch to pane
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.selectPane', async (arg: TmuxTreeItem | { connectionId: string; paneId: string }) => {
            let connectionId: string;
            let paneId: string;

            // Support two call modes: double-click passes a simple object, context menu passes a TmuxTreeItem
    ...
    // Close panes (supports multi-select batch deletion)
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.killPane', async (item: TmuxTreeItem, selectedItems?: TmuxTreeItem[]) => {
            // Get the list of panes to close
            // Use selected items when multi-selecting; otherwise use the clicked item
            const panesToKill: TmuxTreeItem[] = [];
            
            if (selectedItems && selectedItems.length > 1) {
                // Multi-select mode: filter all pane items
                for (const selected of selectedItems) {
            ...
            // Sort by pane ID descending and close from back to front to avoid ID shifts
    ...
    // Swap pane position
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.swapPane', async (item: TmuxTreeItem) => {
            if (!item.data.pane) {
                return;
            }
    ...
    // Resize pane
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.resizePane', async (item: TmuxTreeItem) => {
            if (!item.data.pane) {
                return;
            }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.toggleMouseMode', async (item?: TmuxTreeItem) => {
            const connectionId = item?.data.connectionId || 'local';
    ...
    // Add SSH connection
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.addConnection', async () => {
            // Try reading hosts from SSH config
            ...
            // Offer quick pick when SSH config hosts exist
            if (sshHosts.length > 0) {
                ...
                // If a host from SSH config was selected
            ...
            // Manual entry or additional information
            if (!host) {
            ...
            // Use private-key authentication by default when a private-key path exists
    ...
    // Remove connection
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.removeConnection', async (item: TmuxTreeItem) => {
            const connection = connectionManager.getConnection(item.data.connectionId);
    ...
    // Edit connection
    context.subscriptions.push(
        vscode.commands.registerCommand('tmuxManager.editConnection', async (item: TmuxTreeItem) => {
            const connection = connectionManager.getConnection(item.data.connectionId);
